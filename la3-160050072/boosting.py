...
class AdaBoostClassifier:
    ...
    def train( self, trainingData, trainingLabels):
        ...
        priority = util.normalize([1] * data_size)
        for i in range(self.boosting_iterations):
            self.classifiers[i].train(trainingData , trainingLabels , priority)
            error = 0 
            for k in range(data_size):
                if self.classifiers[i].classify([trainingData[k]])[0] != trainingLabels[k]:
                    error += priority[k]
            for k in range(data_size):
                if self.classifiers[i].classify([trainingData[k]])[0] == trainingLabels[k]:
                    priority[k] *= (error) / (1 - error)
            priority = util.normalize(priority)
            self.alphas[i] = np.log( ( 1 - error)/ error)
            print "Alpha ", i, ": " ,self.alphas[i]
    ...
    def classify( self, data):
        ...
        result = []
        for datum in data:
        	vec = [self.alphas[i] * self.classifiers[i].classify([datum])[0] for i in range(len(self.classifiers))]
        	result.append(util.sign(sum(vec)))
        return result
