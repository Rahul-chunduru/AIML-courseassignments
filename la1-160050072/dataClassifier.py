...
def enhancedFeatureExtractorDigit(datum):
	...
	I came up with the following features after a lot of trial and error.
	The idea was to link the 1d features of the shape such as perimeter, 
	span over mean vs 2d feature area. 
    ##

    These are the used features, 
    		1. No of black pixels
    			This feature gives an idea of the area of the shape in the image
    		2. No of hv corners
    			This is a crude feature, this is a count of number of pixels with 
    			either a non zero horizonal or vertical derivative. This serves as 
    			an approximate to the perimeter of the shape in the feature.
    		3. Circumradius of the figure.
    			This is another intuitive feature. This gives the maximum distance 
    			between the mean of the shape and a pixel in the shape, i.e, the farthest
    			point from mean. This is intuitively a connected to the perimeter and area
    			in a way that depends upon the shape.

    Overall these features achieve an accuracy of 99 % over the dataset using 1vr classifier. 
    """
    ...
    for x in range(DATUM_WIDTH):
    	for y in range(DATUM_HEIGHT):
    		if getPixelVal(x , y) == 1 : 
    			blacknum += 1
    hcorners = 0 
    for x in range( 1, DATUM_WIDTH ) : 
    	for y in range(0 , DATUM_HEIGHT - 1):
    		hcorners += (abs(getPixelVal( x , y) - getPixelVal(x - 1 , y))  > 0)
    vcorners = 0		
    for x in range( 1, DATUM_WIDTH - 1) : 
    	for y in range(0 , DATUM_HEIGHT ):
    		vcorners += (abs(getPixelVal( x , y) -  getPixelVal(x  , y - 1))  > 0)    		
    features[0] = blacknum 
    # features[1] = hcorners
    # features[2] = vcorners
    # gradxcount = 0 
    # for x in range( 1, DATUM_WIDTH - 1) : 
    # 	for y in range(0 , DATUM_HEIGHT - 1):
    # 		gradxcount += ((getPixelVal( x + 1, y) * 2  + getPixelVal(x + 1 , y + 1) * 1
  		# + getPixelVal(x + 1 , y - 1) * 1 - getPixelVal(x - 1, y) * 2 - getPixelVal(x - 1 , y -1 ) 
  		# - getPixelVal(x - 1 , y + 1) ) > 0)
    # gradycount = 0 
    # for x in range( 1, DATUM_WIDTH - 1) : 
    # 	for y in range(0 , DATUM_HEIGHT - 1):
    # 		gradycount += ((getPixelVal( x , y + 1) * 2  + getPixelVal(x + 1 , y + 1) * 1
  		# + getPixelVal(x - 1 , y + 1) * 1 - getPixelVal(x, y - 1) * 2 - getPixelVal(x - 1 , y -1 ) 
  		# - getPixelVal(x + 1 , y - 1) ) > 0)		
    # features[2] = gradxcount
    # features[3] = gradycount 	
    hvcorners = 0
    for x in range( 1, DATUM_WIDTH - 1) : 
    	for y in range(0 , DATUM_HEIGHT -1 ):
    		hvcorners += ((abs(getPixelVal( x , y + 1) -  getPixelVal(x  , y - 1))) ** 2  + 
    		(abs(getPixelVal( x + 1 , y) - getPixelVal(x - 1 , y))) ** 2)  
    features[1] = hvcorners	  
    xmean = 0 
    ymean = 0 
    for x in range(DATUM_WIDTH):
    	for y in range(DATUM_HEIGHT):
    		if getPixelVal(x , y ) == 1:
    			xmean += x
    			ymean += y
    xmean = xmean / (blacknum)
    ymean = ymean / (blacknum)
    maxdist = 0
    for x in range(DATUM_WIDTH):
    	for y in range(DATUM_HEIGHT):
    		if getPixelVal(x , y ) == 1:
    			maxdist = max(maxdist , (x - xmean) ** 2 + (y - ymean) ** 2)
    features[2] = maxdist
    # print features 			
    return features
