	...
	def train( self, trainingData, trainingLabels, validationData, validationLabels, validate):
		...
		for iteration in range(self.max_iterations):
			...
			for i in range(len(trainingData)):
				...
				yactual = trainingLabels[i] 
				for j in range(yactual):
					W = self.weights[frozenset([j , yactual])]
					if W * trainingData[i] > 0:
						continue
					else:
						self.weights[frozenset([j , yactual])] = W + trainingData[i]
				for j in range(yactual + 1 , len(self.legalLabels)):
					W = self.weights[frozenset([yactual , j])]
					if W * trainingData[i] <= 0:
						continue
					else:
						self.weights[frozenset([yactual , j])] = W - trainingData[i] 
